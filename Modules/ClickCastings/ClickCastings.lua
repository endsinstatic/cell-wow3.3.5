...
-- NOTE: 3.3.5a restricted execution environment doesn't support self:Run(),
-- table.new(), and other modern secure handler methods. Disable secure
-- click-casting snippets to prevent keyboard taint.
-- State drivers and wrap scripts are skipped entirely on 3.3.5a.
local _is335 = (select(4, GetBuildInfo()) or 99999) < 40000
if not _is335 then
...
if _is335 then
    -- 3.3.5a: enable keyboard/scroll binding via _onenter/_onleave only.
    -- We skip WrapScript and RegisterStateDriver (those cause keyboard taint),
    -- but the SetBindingClick calls in _onenter are safe in the restricted env.
    -- This allows Razer Naga side buttons (which send number keys) to work
    -- as click-cast bindings on Cell unit frames.
    SetBindingClicks = function(b)
        local snippet = b:GetAttribute("snippet") or ""
        if snippet == "" then return end

        b:SetAttribute("_onenter", [[
            self:SetAttribute("cell335hover", true)
            self:ClearBindings()
            ]] .. snippet .. [[
        ]])
        b:SetAttribute("_onleave", [[
            self:SetAttribute("cell335hover", nil)
            self:ClearBindings()
        ]])
        b:SetAttribute("_onhide", [[
            self:ClearBindings()
        ]])
        b:SetAttribute("_onshow", [[
            if self:GetAttribute("cell335hover") then
                self:ClearBindings()
                ]] .. snippet .. [[
            end
        ]])
...
elseif Cell.isRetail then
    ...
    SetBindingClicks = function(b)
        -- NOTE: self:Run() doesn't exist in 3.3.5a secure handlers.
        -- Inline the snippet directly into the _onenter body instead.
        local snippet = b:GetAttribute("snippet") or ""
        b:SetAttribute("_onenter", [[
            -- print("_onenter")
            self:ClearBindings()
...
local function ApplyClickCastings(b)
    for i, t in pairs(clickCastingTable) do
        ...
        if t[2] == "spell" then
            ...
            if t[3] == 428332 then
                local subtext = C_Spell and C_Spell.GetSpellSubtext and C_Spell.GetSpellSubtext(428332)
            ...
            -- 3.3.5a: "@cell" is a placeholder that gets rewritten by the
            -- secure _onenter handler, but that handler is disabled on 3.3.5a
            -- to avoid keyboard taint. Use "@mouseover" which naturally
            -- resolves to the unit under the cursor when clicking.
            local unit = (Cell.isRetail or _is335) and "@mouseover" or "@cell"
            ...
            -- /stopspelltarget doesn't exist on 3.3.5a
            local fix = (t[3] == 370665 or _is335) and "" or "\n/stopspelltarget"
