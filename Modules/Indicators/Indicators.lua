...
local function InitIndicator(indicatorName)
    ...
    elseif indicatorName == "privateAuras" then
        ...
        if indicator.cooldown.SetDrawEdge then indicator.cooldown:SetDrawEdge(false) end
        if indicator.cooldown.SetDrawBling then indicator.cooldown:SetDrawBling(false) end
...
local function UpdateIndicators(layout, indicatorName, setting, value, value2)
    ...
    if not indicatorName then -- init
        if layout == true then --! call from UpdateIndicators(true) not from Cell.Fire("UpdateIndicators", ...)
            ...
            for i, t in pairs(currentLayoutTable["indicators"]) do
                ...
                if t["privateAuraOptions"] then
                    if indicator.cooldown.SetDrawSwipe then indicator.cooldown:SetDrawSwipe(t["privateAuraOptions"][1]) end
                    if indicator.cooldown.SetHideCountdownNumbers then indicator.cooldown:SetHideCountdownNumbers(not (t["privateAuraOptions"][1] and t["privateAuraOptions"][2])) end
                end
                -- update glow
        ...
        elseif setting == "privateAuraOptions" then
            if indicator.cooldown.SetDrawSwipe then indicator.cooldown:SetDrawSwipe(value[1]) end
            if indicator.cooldown.SetHideCountdownNumbers then indicator.cooldown:SetHideCountdownNumbers(not (value[1] and value[2])) end
        ...
        if CellDB["indicatorPreview"]["showAll"] then
            if i:IsObjectType("Texture") or i:IsObjectType("FontString") then
                ...
                if i._PixelGlow then
                    if not i._PixelGlow.SetIgnoreParentAlpha then
                        Cell335_PatchFrame(i._PixelGlow)
                    end
