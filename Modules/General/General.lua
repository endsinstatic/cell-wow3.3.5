...
local function ShowTab(tab)
    if tab == "general" then
        if not init then
            ...
            -- easter egg for a very special (and useless) old man
            do
                local name = UnitName("player")
                if name == "Axxi" then
                    local eggPane = Cell.CreateTitledPane(generalTab, "Special Options for Useless Old Men", 205, 50)
                    eggPane:SetPoint("TOPLEFT", generalTab, "TOPLEFT", 222, -420)

                    local wisdomCB = Cell.CreateCheckButton(eggPane, "Enable Extra Wisdom", function(checked)
                        if checked then
                            print("|cffffcc00[Cell]|r Wisdom enabled. You're still useless, but now with |cff00ff00extra wisdom|r.")
                        else
                            print("|cffffcc00[Cell]|r Wisdom disabled. Back to regular uselessness.")
                        end
                    end)
                    wisdomCB:SetPoint("TOPLEFT", eggPane, 5, -27)
                end
            end
