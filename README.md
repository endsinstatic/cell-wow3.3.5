...
**Converted to WotLK 3.3.5a by DrDrty**
...
### r274-335b
- Fixed incoming heal prediction — bundled LibHealComm-4.0, enabled by default on 3.3.5a
- Fixed crash when querying HoT heal amounts (nil `OVERTIME_AND_BOMB_HEALS` constant)

### r274-335a
- Fixed keyboard click-cast bindings dropping after repeated use (hide/show cycle clearing override bindings)
- Fixed druid action bar keys being stolen by Cell when not hovering frames (stale binding recovery)
- Removed `/stopspelltarget` from spell macros (doesn't exist on 3.3.5a)
- Added `/cell ver` slash command

### r274-335 (initial port)
- 1400+ line `Compat335.lua` shim layer for missing APIs
- Event translation (GROUP_ROSTER_UPDATE, UNIT_POWER_FREQUENT, etc.)
- UnitBuff/UnitDebuff wrappers to strip extra `rank` return value
- Secure frame taint avoidance (per-instance patching, no metatable modification on Frame types)
- ChromieCraft-specific fixes (UnitClassBase, macro conditionals, party frame state drivers)
- Click-casting keyboard support via secure `_onenter`/`_onleave` handlers
