...
local _AceComm = LibStub:GetLibrary("AceComm-3.0")

-- 3.3.5a SendCommMessage guard
-- The vanilla 3.3.5a SendAddonMessage rejects chat types it doesn't recognize
-- (anything outside PARTY/RAID/BATTLEGROUND/GUILD/OFFICER/WHISPER), and it
-- also fails silently when the player isn't actually in the channel they're
-- targeting. We proxy AceComm-3.0 so invalid sends are dropped instead of
-- producing the "Unknown addon chat type" popup.
local _validChatTypes = {
    PARTY = true, RAID = true, BATTLEGROUND = true,
    GUILD = true, OFFICER = true, WHISPER = true,
}
local function _canSend(distribution)
    if not _validChatTypes[distribution] then return false end
    if distribution == "PARTY" and (GetNumPartyMembers() or 0) == 0 then return false end
    if distribution == "RAID" and (GetNumRaidMembers() or 0) == 0 then return false end
    if distribution == "GUILD" or distribution == "OFFICER" then
        if not IsInGuild() then return false end
    end
    return true
end
local Comm = setmetatable({}, {__index = _AceComm})
function Comm:SendCommMessage(prefix, message, distribution, target, priority, callback)
    if not _canSend(distribution) then return end
    return _AceComm.SendCommMessage(_AceComm, prefix, message, distribution, target, priority, callback)
end
function Comm:RegisterComm(prefix, callback)
    return _AceComm.RegisterComm(_AceComm, prefix, callback)
end
...
local function UpdateSendChannel()
    -- 3.3.5a: INSTANCE_CHAT / LE_PARTY_CATEGORY_INSTANCE don't exist.
    -- LFG dungeon groups use PARTY/RAID like normal groups.
    if not Cell.isWrath and LE_PARTY_CATEGORY_INSTANCE and IsInGroup(LE_PARTY_CATEGORY_INSTANCE) then
...
function eventFrame:GROUP_ROSTER_UPDATE()
    if IsInGroup() then
        Cell335_UnregisterEvent(eventFrame, "GROUP_ROSTER_UPDATE")
        UpdateSendChannel()
