...
local function Shared_CreateCooldown_Vertical(frame)
    ...
    if cooldown.SetReverseFill then cooldown:SetReverseFill(true) end
    ...
    -- MaskTexture is not available in 3.3.5a; skip the mask and just overlay the icon
    local icon = cooldown:CreateTexture(nil, "ARTWORK")
    cooldown.icon = icon
    -- icon:SetTexCoord(0.12, 0.88, 0.12, 0.88)
    icon:SetDesaturated(true)
    icon:SetAllPoints(frame.icon)
    icon:SetVertexColor(0.6, 0.6, 0.6, 0.6)
end
...
local function Shared_CreateCooldown_Clock(frame)
    ...
    if cooldown.SetSwipeTexture then cooldown:SetSwipeTexture(Cell.vars.whiteTexture) end
    if cooldown.SetSwipeColor then cooldown:SetSwipeColor(0, 0, 0, 0.77) end
...
function I.CreateAura_BorderIcon(name, parent, borderSize)
    ...
    if cooldown.SetSwipeTexture then cooldown:SetSwipeTexture(Cell.vars.whiteTexture) end
    if cooldown.SetSwipeColor then cooldown:SetSwipeColor(1, 1, 1) end
    if cooldown.SetHideCountdownNumbers then cooldown:SetHideCountdownNumbers(true) end
    -- disable omnicc
    cooldown.noCooldownCount = true
    ...
    t1:SetDuration(0.1)
    t1:SetOrder(1)
    if t1.SetSmoothing then t1:SetSmoothing("OUT") end
    local t2 = ag:CreateAnimation("Translation")
    t2:SetOffset(0, -5)
    t2:SetDuration(0.1)
    t2:SetOrder(2)
    if t2.SetSmoothing then t2:SetSmoothing("IN") end
