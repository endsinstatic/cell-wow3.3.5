...
local function PrivateAuras_UpdatePrivateAuraAnchor(self, unit)
    -- Private Auras are retail-only (C_UnitAuras not available in 3.3.5a)
...
function I.CreateAggroBlink(parent)
    ...
    blink.alpha = alpha
    if alpha.SetFromAlpha then alpha:SetFromAlpha(1) end
...
function I.CreatePowerWordShield(parent)
    ...
    if shieldAmount.SetSwipeTexture then shieldAmount:SetSwipeTexture([[Interface\AddOns\Cell\Media\Shapes\circle_filled.tga]]) end
    -- shieldAmount:SetSwipeTexture(Cell.vars.whiteTexture)
    if shieldAmount.SetSwipeColor then shieldAmount:SetSwipeColor(1, 1, 0) end
    shieldAmount.noCooldownCount = true -- disable omnicc
    if shieldAmount.SetHideCountdownNumbers then shieldAmount:SetHideCountdownNumbers(true) end
    ...
    if shieldCooldown.SetSwipeTexture then shieldCooldown:SetSwipeTexture([[Interface\AddOns\Cell\Media\Shapes\circle_filled.tga]]) end
    if shieldCooldown.SetSwipeColor then shieldCooldown:SetSwipeColor(0, 1, 0) end
    shieldCooldown.noCooldownCount = true -- disable omnicc
    if shieldCooldown.SetHideCountdownNumbers then shieldCooldown:SetHideCountdownNumbers(true) end
    shieldCooldown:Hide()
    pcall(shieldCooldown.SetScript, shieldCooldown, "OnCooldownDone", function()
        shieldCooldown:Hide()
    end)
    ...
    if weakendedSoulCooldown.SetSwipeTexture then weakendedSoulCooldown:SetSwipeTexture([[Interface\AddOns\Cell\Media\Shapes\circle_filled.tga]]) end
    if weakendedSoulCooldown.SetSwipeColor then weakendedSoulCooldown:SetSwipeColor(1, 0, 0) end
    weakendedSoulCooldown.noCooldownCount = true -- disable omnicc
    if weakendedSoulCooldown.SetHideCountdownNumbers then weakendedSoulCooldown:SetHideCountdownNumbers(true) end
    weakendedSoulCooldown:Hide()
    pcall(weakendedSoulCooldown.SetScript, weakendedSoulCooldown, "OnCooldownDone", function()
        weakendedSoulCooldown:Hide()
    end)
    ...
    function powerWordShield:SetShape(shape)
        ...
        powerWordShield:SetBackdropColor(0, 0, 0, 0.75)
        if shieldAmount.SetSwipeTexture then shieldAmount:SetSwipeTexture(tex) end
        innerBG:SetTexture(tex, "CLAMP", "CLAMP", "TRILINEAR")
        if shieldCooldown.SetSwipeTexture then shieldCooldown:SetSwipeTexture(tex) end
        if weakendedSoulCooldown.SetSwipeTexture then weakendedSoulCooldown:SetSwipeTexture(tex) end
    end
