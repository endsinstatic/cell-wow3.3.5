...
function P.Size(frame, width, height)
    frame.width = width
    frame.height = height
    local sw, sh = P.Scale(width), P.Scale(height)
    frame:SetSize(sw, sh)
    if Cell and Cell._debugSize then
        print("|cff00ffff[P.Size]|r " .. tostring(frame:GetName() or frame:GetObjectType()) .. " -> " .. sw .. "x" .. sh)
    end
end
...
local function CheckPixelSnap(frame, snap)
    if (frame and not (frame.IsForbidden and frame:IsForbidden())) and frame.PixelSnapDisabled and snap then
        frame.PixelSnapDisabled = nil
    end
end

local function DisablePixelSnap(frame)
    if (frame and not (frame.IsForbidden and frame:IsForbidden())) and not frame.PixelSnapDisabled then
...
local function UpdateMetatable(obj)
    ...
    if not obj.DisabledPixelSnap and (t.SetSnapToPixelGrid or t.SetStatusBarTexture or t.SetTexture or t.SetVertexColor or t.CreateTexture or t.SetTexCoord or t.SetTexture) then
...
pcall(UpdateMetatable, obj:CreateMaskTexture())