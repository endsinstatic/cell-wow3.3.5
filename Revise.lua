...
function F.Revise()
    ...
    if CellDB["revise"] and dbRevision < 224 then
        ...
        if not strfind(CellDB["snippets"][0]["code"], "CELL_USE_LIBHEALCOMM") then
            local defaultVal = (select(4, GetBuildInfo()) or 99999) < 40000 and "true" or "false"
            CellDB["snippets"][0]["code"] = CellDB["snippets"][0]["code"].."\n\n-- use LibHealComm (boolean, non-retail)\nCELL_USE_LIBHEALCOMM = "..defaultVal
        elseif (select(4, GetBuildInfo()) or 99999) < 40000 then
            CellDB["snippets"][0]["code"] = CellDB["snippets"][0]["code"]:gsub("CELL_USE_LIBHEALCOMM = false", "CELL_USE_LIBHEALCOMM = true")
        end
