...
local hoverFrame = CreateFrame("Frame", nil, quickAssistFrame)
...
function CellQuickAssist_OnLoad(button)
    ...
    -- targetHighlight
    local targetHighlight = CreateFrame("Frame", nil, button)
    ...
    -- mouseoverHighlight
    local mouseoverHighlight = CreateFrame("Frame", nil, button)
    ...
    for i = 1, 5 do
        if buffIcons[i].cooldown:IsObjectType("StatusBar") then
            ...
            hooksecurefunc(buffIcons[i], "SetCooldown", function(self, _, _, _, _, _, _, color, glow)
                self.tex:SetTexture(unpack(color))
                -- self.spark:SetTexture(color[1], color[2], color[3], 1) -- ignore alpha
