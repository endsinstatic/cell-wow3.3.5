...
local function UpdateLayoutPreview()
    if not layoutPreviewFrame then
        ...
        for i = 1, 40 do
            layoutPreviewButtons[i] = CreateFrame("Frame", nil, layoutPreviewFrame)
...
local function CreateNameFilter(parent)
    ...
    nameListFrame = CreateFrame("Frame", nil, parent)
...
local function CreateAutoSwitchFrame()
    ...
    mythicText:SetText((_G.RAID or "Raid").." "..(_G.PLAYER_DIFFICULTY6 or "Mythic"))
...
local function CreateIconOptions(parent)
    iconOptionsFrame = CreateFrame("Frame", nil, parent)
...
local function CreateBarOptions(parent)
    barOptionsFrame = CreateFrame("Frame", nil, parent)
...
local function CreateGlowOptions(parent)
    glowOptionsFrame = CreateFrame("Frame", nil, parent)
...
local function CreateSpellsPane()
    ...
    if not CellDB["quickAssistHelpViewed"] then
        local helpFrame = CreateFrame("Frame", nil, pages.spell)
        ...
        helpText:SetFont((UNIT_NAME_FONT_CHINESE or GameFontNormal:GetFont()), 12 + CellDB["appearance"]["optionsFontSizeOffset"], "")
