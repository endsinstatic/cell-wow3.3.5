...
local function Send(msg)
    ...
    if Cell.hasHighestPriority then
        if not Cell.isWrath and IsInGroup(LE_PARTY_CATEGORY_INSTANCE) then
...
function frame:PLAYER_ENTERING_WORLD()
    ...
    if instanceType == "pvp" or instanceType == "arena" then
        ...
        Cell335_UnregisterEvent(frame, "GROUP_ROSTER_UPDATE")
        return
    else
        Cell335_RegisterEvent(frame, "GROUP_ROSTER_UPDATE")
    end
