...
function U.CreateDispelRequestText(parent)
    ...
    if tex.SetParentKey then tex:SetParentKey("Flipbook") end

    local ag = drText:CreateAnimationGroup()
    ag:SetLooping("REPEAT")

    -- FlipBook animation is retail-only; skip on 3.3.5a
    local ok_fb, flip = pcall(ag.CreateAnimation, ag, "FlipBook")
    if not ok_fb then flip = nil end
    if flip then
        flip:SetDuration(1)
        if flip.SetFlipBookRows then
            flip:SetFlipBookRows(8)
            flip:SetFlipBookColumns(4)
            flip:SetFlipBookFrames(31)
        end
        if flip.SetChildKey then flip:SetChildKey("Flipbook") end
    end
    ...
    function drText:SetType(type)
        ...
        if flip and flip.SetFlipBookFrames then flip:SetFlipBookFrames(flipBookFrames[type]) end
    end
