...
for i = 1, 8 do
    ...
    -- NOTE: RunAttribute / RunFor don't exist in 3.3.5a secure handlers.
    -- NPC/boss frames aren't used in 3.3.5a content, so skip secure snippets.
...
-------------------------------------------------
-- NOTE: 3.3.5a secure handlers don't support RunAttribute/RunFor.
-- NPC frame (boss frames) aren't used in 3.3.5a content.
-- State handler snippets completely removed to avoid corrupting
-- secure state and locking keyboard input.
...
local function NPCFrame_UpdateLayout(layout, which)
    -- 3.3.5a: NPC/boss frames are fully disabled (boss unit tokens don't exist,
    -- secure state drivers corrupt keyboard input). Skip all layout work to
    -- avoid "memory allocation error: block too big" on the older Lua VM.
    ...
    if not which or which == "npc" then
        -- NOTE: NPC/boss frames disabled on 3.3.5a — boss unit tokens don't exist
        -- and secure state drivers with RunAttribute/RunFor corrupt keyboard input.
        for _, b in ipairs(Cell.unitButtons.npc) do
            b:Hide()
        end
        npcFrame:Hide()
