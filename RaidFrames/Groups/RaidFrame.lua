...
-- 3.3.5a: SecureGroupHeaderTemplate creates children as child frames but
-- does NOT store them at header[1], header[2], etc. on 3.3.5a.
-- Sync from GetChildren() so ipairs(header) works.
local function SyncRaidHeaderChildren(header)
    ...
    for i, child in ipairs(children) do
        -- only sync CellUnitButtonTemplate children (skip helpers etc.)
        if child.GetAttribute and child:GetAttribute("unit") ~= nil or
           (child:GetName() and string.find(child:GetName(), "UnitButton")) then
            count = count + 1
            header[count] = child
        end
    end
...
-- Sync children + create visibility helpers after all headers exist
local function InitRaidHeaders()
    for group = 1, 8 do
        local header = separatedHeaders[group]
        SyncRaidHeaderChildren(header)
        -- Create visibility helper if first button exists
        if header[1] and not header._helperCreated then
            header._helperCreated = true
...
local function RaidFrame_UpdateLayout(layout, which)
    ...
    if Cell.vars.groupType ~= "raid" or Cell.vars.isHidden then
        ...
        -- 3.3.5a: sync header children (may have been created since load)
        InitRaidHeaders()
        -- Deferred init for indicators/appearance (same as PartyFrame)
        if not raidInitDone then
            raidInitDone = true
            C_Timer.After(0.5, function()
                Cell.Fire("UpdateAppearance")
                B.ForceInitButtons()
                if Cell.vars.currentLayout then
