...
-- Sync from GetChildren() so ipairs(header) and header[j] work.
local function SyncHeaderChildren()
    local children = {header:GetChildren()}
    -- Sort by name to get stable order (UnitButton1, UnitButton2, ...)
...
-- Try at load time (modern WoW creates children immediately)
SyncHeaderChildren()
InitPetButtons()
...
local function PartyFrame_UpdateLayout(layout, which)
    ...
    if Cell.vars.groupType ~= "party" or Cell.vars.isHidden then
        ...
        -- 3.3.5a: @unit conditionals don't exist; use simple group conditionals
        if Cell.is335 then
    ...
    if not which or which == "main-arrangement" or which == "pet-arrangement" then
        ...
        for j = 1, 5 do
            if not header[j] then break end -- 3.3.5a: buttons may not be pre-created
