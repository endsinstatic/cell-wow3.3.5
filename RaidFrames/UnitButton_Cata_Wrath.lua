local _, Cell = ...
local L = Cell.L
local CreateFrame = Cell335_CreateFrame or CreateFrame -- 3.3.5a: auto-patch frames
---@type CellFuncs
local F = Cell.funcs
...
-- 3.3.5a: direct init bypass for late-created buttons (PartyFrame SyncHeaderChildren)
...
local function UnitButton_UpdateDebuffs(self)
    ...
    for i = 1, 40 do
        ...
        local name, icon, count, debuffType, duration, expirationTime, source, _, _, spellId = Cell335_UnitDebuff(unit, i)
        if not name then
            break
        end

        local auraInstanceID = (source or "") .. (spellId or name or "")
    ...
    if raidDebuffsFound then
        ...
        for i = 1, indicatorNums["raidDebuffs"] do
            ...
            if index then
                local name, icon, count, debuffType, duration, expirationTime, source, isStealable, nameplateShowPersonal, spellId = Cell335_UnitDebuff(unit, self._debuffs_raid[i])
                if name then
    ...
    if enabledIndicators["debuffs"] then
        ...
        for debuffIndex, refreshing in pairs(self._debuffs_big) do
            local name, icon, count, debuffType, duration, expirationTime, _, _, _, spellId = Cell335_UnitDebuff(unit, debuffIndex)
...
local function UnitButton_UpdateBuffs(self)
    ...
    for i = 1, 40 do
        -- name, icon, count, debuffType, duration, expirationTime, source, isStealable, nameplateShowPersonal, spellId, canApplyAura, isBossDebuff, castByPlayer, nameplateShowAll, timeMod, ...
        local name, icon, count, debuffType, duration, expirationTime, source, _, _, spellId, _, _, _, _, _, arg16 = Cell335_UnitBuff(unit, i)
        if not name then
            break
        end
...
CheckPowerEventRegistration = function(b)
    if b:IsVisible() and not b.isPreview and (b._shouldShowPowerText or b._shouldShowPowerBar) then
        Cell335_RegisterEvent(b, "UNIT_POWER_FREQUENT")
        Cell335_RegisterEvent(b, "UNIT_MAXPOWER")
        b:RegisterEvent("UNIT_DISPLAYPOWER")
        return true
    else
        Cell335_UnregisterEvent(b, "UNIT_POWER_FREQUENT")
        Cell335_UnregisterEvent(b, "UNIT_MAXPOWER")
...
local function UnitButton_UpdateHealPrediction(self)
    ...
    if CELL_USE_LIBHEALCOMM and HealComm then
        if self.__displayedGuid then
            ...
            local hotType = bit.bor(HealComm.HOT_HEALS or 0x04, HealComm.BOMB_HEALS or 0x10)
            local hot = HealComm:GetHealAmount(self.__displayedGuid, hotType, GetTime()+3) or 0
            value = (casted + hot) * modifier
            if Cell.debugHeals and (casted > 0 or hot > 0) then
                print("|cff00ff00[Cell]|r HealPred:", unit, "casted=", casted, "hot=", hot, "mod=", modifier)
            end
        end
    else
...
if CELL_USE_LIBHEALCOMM then
    ...
    if HealComm then
        ...
        local function HealComm_UpdateHealPrediction(_, event, casterGUID, spellID, healType, endTime, ...)
            if Cell.debugHeals then
                print("|cff00ff00[Cell]|r HealComm:", event, "spell=", spellID, "targets=", select("#", ...))
            end
            for i = 1, select("#", ...) do
                local guid = select(i, ...)
                if Cell.debugHeals then
                    local mapped = Cell.vars.guids[guid]
                    local found = F.HandleUnitButton("guid", guid, UnitButton_UpdateHealPrediction)
                    print("|cff00ff00[Cell]|r  target:", guid, "unit=", tostring(mapped), "found=", tostring(found))
                else
...
local function UnitButton_RegisterEvents(self)
    ...
    Cell335_RegisterEvent(self, "GROUP_ROSTER_UPDATE")

    self:RegisterEvent("UNIT_HEALTH")
    self:RegisterEvent("UNIT_HEALTH_FREQUENT")
    self:RegisterEvent("UNIT_MAXHEALTH")

    Cell335_RegisterEvent(self, "UNIT_POWER_FREQUENT")
    Cell335_RegisterEvent(self, "UNIT_MAXPOWER")
    ...
    if not success then
        print("|cffff0000[Cell335] UnitButton_UpdateAll FAILED:|r", self:GetName(), result)
    else
        -- if Cell.is335 then print("|cff00ff00[Cell335]|r UnitButton_UpdateAll OK:", self:GetName(), "unit:", tostring(self.states.unit)) end
    end
end
...
local function UnitButton_OnEvent(self, event, unit)
    ...
    if unit and (self.states.displayedUnit == unit or self.states.unit == unit) then
        ...
        elseif event == "UNIT_MAXPOWER" or Cell335_IsMaxPowerEvent(event) then
            UnitButton_UpdatePowerStates(self)
            UnitButton_UpdatePowerMax(self)
            UnitButton_UpdatePower(self)
            UnitButton_UpdatePowerText(self)

        elseif event == "UNIT_POWER_FREQUENT" or Cell335_IsPowerFreqEvent(event) then
            UnitButton_UpdatePowerStates(self)
            UnitButton_UpdatePower(self)
            UnitButton_UpdatePowerText(self)
        ...
        if Cell335_IsGroupRosterEvent(event) then
            self._updateRequired = 1
            self._powerUpdateRequired = 1
...
function B.SetOrientation(button, orientation, rotateTexture)
    ...
    if healthBar.SetRotatesTexture then healthBar:SetRotatesTexture(rotateTexture) end
    if powerBar.SetRotatesTexture then powerBar:SetRotatesTexture(rotateTexture) end
...
function CellUnitButton_OnLoad(button)
    ...
    if alpha.SetFromAlpha then alpha:SetFromAlpha(0.7) end
    if alpha.SetToAlpha then alpha:SetToAlpha(0) end
    ...
    -- 3.3.5a: some clients don't register Button4/5 with just "AnyDown".
    -- Explicitly list them to support multi-button mice (Razer Naga, etc.).
