...
if class then
    ...
    accentColor.t[1], accentColor.t[2], accentColor.t[3] = cc.r, cc.g, cc.b
    accentColor.s = format("|cff%02x%02x%02x", cc.r*255, cc.g*255, cc.b*255)
end
...
font_chinese:SetFont((UNIT_NAME_FONT_CHINESE or GameFontNormal:GetFont()), 14, "")
...
function Cell.UpdateOptionsFont(offset, useGameFont)
    ...
    font_chinese:SetFont((UNIT_NAME_FONT_CHINESE or GameFontNormal:GetFont()), 14+offset, "")
...
function Cell.CreateFrame(name, parent, width, height, isTransparent, template)
    local f = CreateFrame("Frame", name, parent, template and template.."" or "")
    f:Hide()
...
function Cell.CreateButton(parent, text, buttonColor, size, noBorder, noBackground, fontNormal, fontDisable, template, ...)
    local b = CreateFrame("Button", nil, parent, template and template.."" or "")
...
function Cell.CreateSlider(name, parent, low, high, width, step, onValueChangedFn, afterValueChangedFn, isPercentage, ...)
    ...
    -- SetObeyStepOnDrag removed (3.3.5a compat)
    ...
    local oldValue
    slider._programmatic = false -- flag to suppress callbacks during SetValue
    slider:SetScript("OnValueChanged", function(self, value, userChanged)
        ...
        -- 3.3.5a: OnValueChanged has no userChanged param (always nil).
        -- Use _programmatic flag to distinguish SetValue() from user drags.
...
function Cell.CreateSwitch(parent, size, leftText, leftValue, rightText, rightValue, func)
    ...
    t1:SetDuration(0.2)
    if t1.SetSmoothing then t1:SetSmoothing("IN_OUT") end
...
function menu:ShowMenu()
    ...
    -- 3.3.5a: ensure strata/level are correct before showing, in case
    -- the menu was reparented (SetMenuParent) after buttons were created.
    menu:SetFrameStrata("TOOLTIP")
...
function menu:SetMenuParent(parent)
    ...
    menu:SetFrameStrata("TOOLTIP")
    -- 3.3.5a: after reparenting, force a high level so the menu and its
    -- children render above everything else within TOOLTIP strata.
    menu:SetFrameLevel(100)
end
...
function Cell.CreateScrollTextFrame(parent, s, timePerScroll, scrollStep, delayTime, noFadeIn)
    ...
    if alpha1.SetFromAlpha then alpha1:SetFromAlpha(1) end
    if alpha1.SetToAlpha then alpha1:SetToAlpha(0) end
    alpha1:SetDuration(0.5)
    alpha1:SetOrder(1)
    local alpha2 = fadeOutIn:CreateAnimation("Alpha")
    if alpha2.SetFromAlpha then alpha2:SetFromAlpha(0) end
    if alpha2.SetToAlpha then alpha2:SetToAlpha(1) end
...
-- 3.3.5a: strata/level set in LoadItems before Show(), not OnShow
...
function Cell.CreateDropdown(parent, width, dropdownType, isMini, isHorizontal)
    ...
    local function LoadItems()
        ...
        -- 3.3.5a: dropdown list must be TOOLTIP strata to render above the
        -- options frame (DIALOG strata, level 520). Using the menu's own
        -- strata/level fails because menu inherits level 0 in 3.3.5a.
    ...
    menu.button:HookScript("OnClick", function()
        ...
        elseif list:IsShown() then -- list showing by this, hide it
            ...
            if menu.reloadRequired then
                ...
                -- 3.3.5a: ensure strata/level set even when skipping LoadItems
                list.menu = menu
                list:SetFrameStrata("TOOLTIP")
                list:SetFrameLevel(10)
            end
...
local function CreateGrid(parent, text, width)
    ...
    -- 3.3.5a: use parent level + 1 instead of hardcoded 6, otherwise the
    -- options frame (level 520) eats clicks meant for these grids.
    ...
    function grid:IsTruncated()
        -- 3.3.5a: FontString:IsTruncated() doesn't exist; approximate by
        -- comparing the string width against the available region width.
        if grid.text.IsTruncated then
            return grid.text:IsTruncated()
        end
        return grid.text:GetStringWidth() > grid.text:GetWidth()
    end
...
function Cell.CreateBindingListButton(parent, modifier, bindKey, bindType, bindAction)
    ...
    -- 3.3.5a: use parent level + 1 instead of hardcoded 5
