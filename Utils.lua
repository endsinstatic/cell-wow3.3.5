...
Cell.is335 = (select(4, GetBuildInfo()) or 0) < 40000 -- 3.3.5a = 30300
Cell.isCata = WOW_PROJECT_ID == WOW_PROJECT_CATACLYSM_CLASSIC
Cell.isMists = WOW_PROJECT_ID == WOW_PROJECT_MISTS_CLASSIC
Cell.isTWW = LE_EXPANSION_LEVEL_CURRENT and LE_EXPANSION_LEVEL_CURRENT == LE_EXPANSION_WAR_WITHIN or false
...
local UnitClassBase = UnitClassBase or function(unit) return select(2, UnitClass(unit)) end
...
function F.GetClassColorStr(class)
    if class and class ~= "" and RAID_CLASS_COLORS[class] then
        if CUSTOM_CLASS_COLORS and CUSTOM_CLASS_COLORS[class] then
            local cc = CUSTOM_CLASS_COLORS[class]
            if cc.colorStr then
                return "|c"..cc.colorStr
            else
                return format("|cff%02x%02x%02x", cc.r*255, cc.g*255, cc.b*255)
            end
        else
            local c = RAID_CLASS_COLORS[class]
...
if Cell.isWrath or Cell.isTBC or Cell.isVanilla then
    ...
    local GetSpellBookItemName = GetSpellBookItemName or GetSpellName -- 3.3.5a: GetSpellName(i, bookType) returns name, rank
...
if Cell.isRetail then
    ...
    function F.FindDebuffByIds(unit, spellIds)
        local debuffs = {}
        for i = 1, 40 do
            local name, icon, count, debuffType, duration, expirationTime, source, isStealable, nameplateShowPersonal, spellId = Cell335_UnitDebuff(unit, i)
            if not name then
                break
            end
...
local IsItemInRange = (C_Item and C_Item.IsItemInRange) or _G.IsItemInRange or function() return nil end
...
local IsSpellBookKnown = (C_SpellBook and C_SpellBook.IsSpellKnown) or _G.IsSpellKnown or function() return false end

local function IsSpellKnown(spellId)
    return (IsSpellKnownOrOverridesKnown or IsSpellBookKnown)(spellId) or IsSpellBookKnown(spellId)
end
...
local function LoadSpellName(spellID, callback)
    if spellID and IsSpellKnown(spellID) then
        -- Spell:CreateFromSpellID is modern WoW; use GetSpellInfo on 3.3.5a
...
-- RangeCheck debug
-------------------------------------------------
local debug = CreateFrame("Frame", "CellRangeCheckDebug", CellParent)
